...
provide the country - but we are adding it as a convenience.

We immediately hit snags like "original_collection_location"=>"USA:
Utah" -> "country"=>"SA" which meant we have to execute a proper regex.
