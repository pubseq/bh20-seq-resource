...
* Table of Contents                                                     :TOC:noexport:
 ...
 - [[#normalizing-location-data][Normalizing location data]]
   - [[#shape-expressions-shex][Shape expressions (ShEx)]]
...
3. If GPS coordinates are provided we can use that
4. Tokenize the record and use the Wikidata or Wikipedia search engine

Hopefully that covers 90% of cases correctly.  We still may end up
with misses and wrong data. E.g. it is easy to imagine a sample taken
on Antartica that got sequenced in the USA and gets placed in the
USA. In Wiki-spirit the best solution is to leave it to end-users to
re-place those entries. End-users scale, unlike us. For this we will
create an API.

Note that normalization has to be fully automated to execute regular
updates from the upstream data repositories.
