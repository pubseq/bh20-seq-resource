...
Sometimes the download stops. In that case best to continue with below
and when done reiterate to above 'List PubSeq IDs'. Particularly if
you are fetching a lot of records.
...
#+begin_src sh
python3 normalize-yamlfa.py -s ~/tmp/new-yamlfa-orig.2/state.json --validate MW084447
#+end_src

To write output specify the output dir:

#+begin_src sh
  python3 normalize-yamlfa.py -s ~/tmp/new-yamlfa-orig.2/state.json --validate MW084447 --out ~/tmp/test
#+end_src
